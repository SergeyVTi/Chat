...
constexpr uint block_expend_size_uints = 80;

constexpr uint SHA1HASHLENGTHBYTES = 20;
constexpr uint SHA1HASHLENGTHUINTS = 5;
...
AuthData::AuthData(const char* password, size_t msize_bytes)
	: password_sha1_hash_(sha1(password,msize_bytes)) {
}

uint AuthData::cycle_shift_left(uint val, size_t bit_count) {
	return (val << bit_count | val >> (shift - bit_count));
}

uint AuthData::bring_to_human_view(uint val) {
...
uint* AuthData::sha1(const char* message, size_t msize_bytes) {//uint
